...
void ubloxGPS::processGPSByte(uint8_t c)
{
    ...
    // decode UBX command
    // Modification for TrackerGNSSTester - If valid ubx protocol, don't look for nema but write
    // out all bytes even if 0xff beacause ubx protocol is binary
    if (decodeUbx((uint8_t)c) != DECODE_RESULT_INVALID) {
        Serial.write(c);
        return;
    }
    ...
    if (c != 0xff) {
        // Modification for TrackerGNSSTester
        Serial.write(c);
    }
